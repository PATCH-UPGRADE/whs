from __future__ import annotations # At least until python3.14
from functools import wraps
from typing import Any, Callable, Literal, Optional, ClassVar, TypeVar
from pydantic import  ConfigDict, Field, IPvAnyAddress, model_serializer, model_validator, BaseModel
...
class ModelStore(PydanticSyncStoreRegistry):
    ...
    vm_images = class_store_property('VmImage')
    container_images = class_store_property('ContainerImage')
    ...
    def resolve_image(self, device_type: str, image: Any) -> VmImage | ContainerImage:
        if device_type == 'container':
            ...
            self.store_synchronize(synthesized_image)
    ...
    def load(self) -> 'ModelStore':
        ...
        self.store_synchronize_multiple(self._load_model_file(self.model_dir / 'vm_images.yml', VmImage))
    ...
    def import_yaml(self, yaml_data: str) -> 'ModelStore':
        ...
        for image_id, payload in container_images.items():
            ...
            self.store_synchronize(ContainerImage.model_validate(
                {'id': image_id, **payload},
                context={'format': 'export', 'store': self},
            ))
        ...
        for pcap_id, payload in pcaps.items():
            ...
            self.store_synchronize(Pcap.model_validate(
                {'id': pcap_id, **payload},
                context={'format': 'export', 'store': self},
            ))
...
class StoredBaseModel(StoreInSyncStoreMixin, SynchronizableBaseModel):
    sync_primary_keys: ClassVar[tuple] = tuple()
    sync_registry = ModelStore
...
class IdentifiedModel(StoredBaseModel):
    id: str
    sync_primary_keys: ClassVar[tuple] = ('id',)
